...
namespace TubeStarMetro
{
    ...
    public static class CommentGenerator
    {
        ...
        private async static void PopulateBlackList()
        {
            var file = await Package.Current.InstalledLocation.GetFileAsync(@"Assets\Blacklist.txt");
            string list = await Windows.Storage.FileIO.ReadTextAsync(file);

            _blackList = list.Split(',').ToList();
        }

        public static async void GenerateRealComments(Video video)
        {
            if (!String.IsNullOrEmpty(video.YouTubeVideoId))
            {
                var response = await WebClientHelpers.Download<YouTubeCommentResponse>(YouTubeAPI.GetRandomComments(video.YouTubeVideoId, 50));
                if (response != null && response.Feed != null && response.Feed.Entries != null && response.Feed.Entries.Count > 0)
                {
                    var comments = response.Feed.Entries.Select(f => f.Content.Comment).ToList();

                    //Remove comments with blacklist words
                    foreach (var bannedWord in _blackList)
                    {
                        comments.RemoveAll(c => c.ToLower().Contains(bannedWord.ToLower()));
                    }
                    comments.RemoveAll(c => c == ""); //Remove empty comments

                    video.RealComments = comments;
                }
            }
        }
        ...
        private static string RandomComment(Video video)
        {
            ...
            if (video.RealComments.Count > 25)
                comments.Clear();
            comments.AddRange(video.RealComments);
