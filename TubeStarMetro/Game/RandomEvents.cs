                    ...
                    case RandomEvent.LoseSubscribers:
                        {
                            ...
                            foreach (var channel in Player.Current.Channels)
                            {
                                if (channel != Channel.UnreleasedVideos && channel.Subscribers.Count > 0)
                                {
                                    var count = channel.Subscribers.Count;
                                    int loss = RandomHelpers.RandomInt(Math.Max(1, (int)(count * 0.2)));
                                    total += loss;

                                    var temp = channel.Subscribers.ToList();
                                    temp.RemoveRange(0, loss);

                                    channel.Subscribers = new HashSet<int>(temp);
                                }
                            }
                            if (total > 0)
                            {
                                ShowMessage(String.Format(EnglishStrings.LoseSubscribers.Translate(), total), MessagePicture.Sad);
                            }
                        }
                        break;
                    ...
                    case RandomEvent.BeautyContest:
                        {
                            ...
                            ShowMessage(String.Format(EnglishStrings.BeautyContest.Translate(), 200), MessagePicture.Money);
                        }
                        break;
                    ...
                    case RandomEvent.GainSubscribers:
                        {
                            ...
                            foreach (var channel in Player.Current.Channels)
                            {
                                ...
                                {
                                    var count = channel.Subscribers.Count;
                                    int gain = RandomHelpers.RandomInt(Math.Max(1, (int)(count * 0.2)));
                                    total += gain;

                                    for (int i = 0; i < gain; i++)
                                    {
                                        channel.Subscribers.Add(Player.Current.Internet.RandomUserId);
                                    }
                                }
                            }
                            if (total > 0)
                            {
                                ShowMessage(String.Format(EnglishStrings.GainSubscribers.Translate(), total), MessagePicture.Happy);
                            }
                        }
                        break;
